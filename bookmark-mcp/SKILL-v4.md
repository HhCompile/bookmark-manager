...
| `list_chrome_profiles(browser)` | — | 列出本机浏览器的 profile（辅助 sync_from_chrome 选择参数） |
| `sync_from_chrome(browser, profile, dry_run)` | `POST /v1/bookmarks/sync-batch` | 从本地浏览器读取书签并推送到 admin（无需 Chrome 扩展，降级方案） |
...
不需要安装 Chrome 扩展，直接读取本机浏览器的 Bookmarks JSON 文件并推送到 admin。

**支持浏览器**：Chrome / Edge / Brave / Arc（macOS / Windows / Linux）

**典型流程**：

```
1. 用户：完全关闭浏览器（⌘Q / Alt+F4）
2. 调用 list_chrome_profiles(browser="chrome")  // 确认 profile 列表
3. 调用 sync_from_chrome(browser="chrome", profile="Default", dry_run=true)
   → 预览将读取多少条，前 5 条样本
4. 确认无误后：sync_from_chrome(browser="chrome", profile="Default")
   → 批量推送到 admin 的 /v1/bookmarks/sync-batch
5. 调用 sync_status() 验证 from_extension 计数增加
```

**注意**：
- ⚠️ 浏览器必须**完全关闭**，否则读到损坏数据（SQLite WAL 锁冲突）
- 重复推送是**幂等**的（基于 URL 哈希去重）
- 支持 dry_run 预览，不实际推送
